...
(function($, window, undefined) {
	...
	var _ie6 = window.ActiveXObject && !window.XMLHttpRequest, _fn = function() {
	}, _count = 0, _rurl = /^url:/, _singleton, onKeyDown, document = window.document, expando = 'JDG' + (+new Date),

	dialogTpl = '<table class="ui_border">' + '<tbody>' + '<tr>' + '<td class="ui_lt"></td>' + '<td class="ui_t"></td>' + '<td class="ui_rt"></td>' + '</tr>' + '<tr>' + '<td class="ui_l"></td>' + '<td class="ui_c">' + '<div class="ui_inner">' + '<table class="ui_dialog">' + '<tbody>' + '<tr>'
			+ '<td colspan="2">' + '<div class="ui_title_bar">' + '<div class="ui_title" unselectable="on"></div>' + '<div class="ui_title_buttons">' + '<a class="ui_min" href="javascript:void(0);" title="\u6700\u5C0F\u5316"><b class="ui_min_b"></b></a>'
			+ '<a class="ui_max" href="javascript:void(0);" title="\u6700\u5927\u5316"><b class="ui_max_b"></b></a>' + '<a class="ui_res" href="javascript:void(0);" title="\u8FD8\u539F"><b class="ui_res_b"></b><b class="ui_res_t"></b></a>'
			+ '<a class="ui_close" href="javascript:void(0);" title="\u5173\u95ED(esc\u952E)">\xd7</a>' + '</div>' + '</div>' + '</td>' + '</tr>' + '<tr>' + '<td class="ui_icon"></td>' + '<td class="ui_main">' + '<div class="ui_content"></div>' + '</td>' + '</tr>' + '<tr>' + '<td colspan="2">'
			+ '<div class="ui_buttons"></div>' + '</td>' + '</tr>' + '</tbody>' + '</table>' + '</div>' + '</td>' + '<td class="ui_r"></td>' + '</tr>' + '<tr>' + '<td class="ui_lb"></td>' + '<td class="ui_b"></td>' + '<td class="ui_rb"></td>' + '</tr>' + '</tbody>' + '</table>',

	/*
	 * ! _path 获取组件核心文件lhgdialog.js所在的绝对路径 _args
	 * 获取lhgdialog.js文件后的url参数组，如：lhgdialog.js?self=true&skin=aero中的?后面的内容
	 */
	...
	/*
	 * ! 获取url参数值函数 @param {String} @return {String||null} @demo
	 * lhgdialog.js?skin=aero | _getArgs('skin') => 'aero'
	 */
	...
	/*
	 * ! IE6下Fixed无抖动静止定位 如果你的页面的html元素设定了背景图片请把设置背景图片的css写到body元素上
	...
	lhgdialog.fn = lhgdialog.prototype = {
		 ...
		 * 自定义按钮
		 * 
		 * @example button({ name: 'login', callback: function(){}, disabled:
		 *          false, focus: true }, .., ..)
		 */
		button : function() {
		...
		/*
		 * ! 设置内容 @param {String} 内容 (如果内容前3个字符为‘url:’就加载单独页面的内容页) @param
		 * {Boolean} 是否为后增加的内容 @param {Boolean} 是否使用iframe方式加载内容页 @return {this}
		 * 如果无参数则返回对象本身
		 */
		content : function(msg, add, frm) {
			...
			if (frm) {
				...
				var addr = msg.split('url:')[1];
				if (addr.indexOf('?') > 0)
					addr += '&ajaxexception=1';
				else
					addr += '?ajaxexception=1';
				that._iframe(addr);
			} else
		...
		position : function(left, top) {
			var that = this, config = that.config, wrap = that.DOM.wrap[0], style = wrap.style, isFixed = _ie6 ? false : config.fixed, ie6Fixed = _ie6 && config.fixed, docLeft = _$top.scrollLeft(), docTop = _$top.scrollTop(), dl = isFixed ? 0 : docLeft, dt = isFixed ? 0 : docTop, ww = _$top.width(), wh = _$top
					.height(), ow = wrap.offsetWidth, oh = wrap.offsetHeight;
		...
		/*
		 * ! 定时关闭 @param {Number} 单位为秒, 无参数则停止计时器 @param {Function} 关闭窗口前执行的回调函数
		 */
		...
		unlock : function() {
			...
			if (mask && that._lock) {
				...
				if (config.parent && config.parent._lock) {
					//IE非兼容下遮罩层无法取除
					if (_ie6 || !+[ 1, ] || document.all || navigator.appName == 'Microsoft Internet Explorer') {
		...
		/*
		 * ! 获取指定id的窗口对象或窗口中iframe加载的内容页的window对象 @param {String} 指定的id @param
		 * {String} 是否返回的为指定id的窗口对象 用数字1来表示真，如果不写或写其它为false @return
		 * {Object|null}
		 */
		...
		/*
		 * ! px与%单位转换成数值 (百分比单位按照最大值换算) 其他的单位返回原值
		 */
		...
		_reset : function() {
			...
			if (test && Math.abs(oldWidth - newWidth) === 17)
				return;
	...
	/*
	 * ! 全局快捷键 由于跨框架时事件是绑定到最顶层页面，所以当当前页面卸载时必须要除移此事件
	...
	onKeyDown = function(event) {
		...
		if (!api || !api.config.esc || api.config.cancel === false)
			return;
	...
	_use = function(event) {
		var limit, startWidth, startHeight, startLeft, startTop, isResize, api = lhgdialog.focus, config = api.config, DOM = api.DOM, wrap = DOM.wrap[0], title = DOM.title, main = DOM.main[0], _dragEvent = lhgdialog.dragEvent,
	...
	$(function() {
		...
		lhgdialog.setting.extendDrag && (function(dragEvent) {
			...
			dragEvent.end = function() {
				...
				style.display = 'none';
				if (api)
					api.DOM.main[0].style.visibility = 'visible';
			};
		})(lhgdialog.dragEvent);
	});
...
(function($, lhgdialog, undefined) {
	 ...
	 *            消息图标
	 */
	lhgdialog.mxalert = function(title, content, icon, callback, parent) {
		// 自定义标题
		if (!title) {
			title = '提示';
		}
	 ...
	 *            取消按钮回调函数
	 */
	lhgdialog.mxconfirm = function(title, content, icon, yes, no, parent) {
		// 自定义标题
		if (!title) {
			title = '确认';
		}
